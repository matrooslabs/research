...
def load_binance_1s_csv(path: str, tz: str = "UTC") -> pd.DataFrame:
    """
    ...
    if 'close' not in df.columns:
        # fallback: try index 4 as 'close'
        df['close'] = pd.to_numeric(df.iloc[:,4], errors='coerce')
    out = pd.DataFrame({'ts': ts, 'close': pd.to_numeric(df['close'], errors='coerce')}).dropna()
...
class DeSeasonalizer:
    ...
    def _factor(self, ts: pd.DatetimeIndex, quarticity: bool=False) -> np.ndarray:
...
class HARQForecaster:
    ...
    def __init__(self):
        self.coef = None
        self.cols = ['c','lr_1','lr_5','lr_60','lr_1440','lRQ5','int']
...
def build_minute_table(
    ...
    """
    df_px_1s: index in seconds, column 'close'.
    ...
    if estimator.deseason is not None:
        ...
        if fit_season_on:
            mask = (g.index>=pd.Timestamp(fit_season_on[0])) & (g.index<pd.Timestamp(fit_season_on[1]))
