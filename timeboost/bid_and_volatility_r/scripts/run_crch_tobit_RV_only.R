# scripts/run_crch_tobit_RV_only.R
# Heteroskedastic Tobit with Student's t errors in crch
# amount ~ beta0 + beta_rv * RV + eps
# log(scale) = gamma0 + gamma_rv*log(RV)
# Censoring: left-censored at reserve (constant or row-specific). Here we keep constant = 1e15 by default.
...
MODEL_PATH <- file.path(OUT_DIR, "crch_tobit_model_RV_only.rds")
REPORT_PATH <- file.path(OUT_DIR, "model_summary_RV_only.txt")
...
# Censoring setup
# -----------------------------
# Constant left-censor limit. Change to NA to auto-detect or to a numeric to force.
LEFT_CENSOR <- 1e15
eps <- 1e-12
...
sink(REPORT_PATH)
cat("===== Heteroskedastic Tobit (crch) with Student's t errors — RV only =====\n\n")
print(summary(fit))
cat("\n\n-- Notes --\n")
cat("* Location (mean) model: amount ~ RV\n")
cat("* Scale model: log(sigma) ~ log(RV)\n")
...
p1 <- ggplot(df, aes(x = pred_resp, y = amount, color = factor(is_censored))) +
  ...
  labs(
    title = "Observed vs Predicted (response scale), RV-only Tobit",
...
# 2) Estimated scale (sigma) vs log RV
p2 <- ggplot(df, aes(x = lRV, y = pred_sig_s)) +
  geom_point(alpha = 0.65) +
  labs(
    title = "Estimated scale (σ) vs log(RV)",
    x = "log(RV)",
    y = "σ (predicted)"
  ) +
  theme_minimal()
...
p3 <- ggplot(df, aes(x = std_latent_resid)) +
  ...
  labs(
    title = "Std. latent residuals (RV-only)",
    x = "(amount - μ̂)/σ̂",
    y = "Count"
  ) +
  theme_minimal()

# 4) Partial dependence: vary RV across quantiles
...
p4a <- ggplot(grid, aes(x = RV_s, y = mu_line)) +
  ...
  labs(
    title = "Partial effect on latent mean μ (RV-only)",
    x = "RV (scaled)", y = "μ̂"
  ) +
  theme_minimal()
...
p4b <- ggplot(grid, aes(x = RV_s, y = sigma_line)) +
  ...
  scale_x_continuous(labels = label_number(scale_cut = cut_short_scale())) +
  labs(
    title = "Partial effect on scale σ (RV-only)",
    x = "RV (scaled)", y = "σ̂"
  ) +
  theme_minimal()

# Save plots (distinct filenames)
ggsave(file.path(FIG_DIR, "obs_vs_pred_RV_only.png"), p1, width = 7.5, height = 5.2, dpi = 150)
ggsave(file.path(FIG_DIR, "sigma_vs_logRV_RV_only.png"), p2, width = 7.5, height = 5.2, dpi = 150)
ggsave(file.path(FIG_DIR, "std_latent_resid_hist_RV_only.png"), p3, width = 7.5, height = 5.2, dpi = 150)
ggsave(file.path(FIG_DIR, "partial_mu_vs_RV_RV_only.png"), p4a, width = 7.5, height = 5.2, dpi = 150)
ggsave(file.path(FIG_DIR, "partial_sigma_vs_RV_RV_only.png"), p4b, width = 7.5, height = 5.2, dpi = 150)

# Combined dashboard (optional)
dash <- cowplot::plot_grid(p1, p2, p3, ncol = 1, rel_heights = c(1.1, 1, 1))
ggsave(file.path(FIG_DIR, "dashboard_basic_RV_only.png"), dash, width = 8, height = 12, dpi = 150)

message("All RV-only plots saved to ./outputs/figures")
